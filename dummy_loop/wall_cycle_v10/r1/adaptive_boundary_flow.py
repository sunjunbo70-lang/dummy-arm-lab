...
VERSION='v10r1.boundary_quadrature.adaptive_support.v4_vertex_compiled'

class QuadratureBudgetError(RuntimeError):
...
def adaptive(begin,end,alpha,beta,area_tol=2.5e-13,max_depth=18,max_nodes=4095,compiled=True):
 if compiled:
  from .compiled_boundary_flow import instantaneous as rate_function
 else:rate_function=instantaneous
 ...
 def sample(lo,hi):
  ...
  for node in (-1/np.sqrt(3),1/np.sqrt(3)):
   ...
   maps,external=rate_function(begin+t*velocity,alpha+t*omega,velocity,omega)
   for kind in (0,1):
 ...
 def visit(lo,hi,coarse,depth):
  ...
  difference=0.;terms=[]
  for kind,(c,f) in enumerate(zip(coarse,fine)):
   ...
   terms.extend(dict(kind=kind,blade=key[0],wall=key[1],signed_difference_m2=float(value)) for key,value in sorted(d.items(),key=lambda kv:abs(kv[1]),reverse=True)[:3])
  ...
  if difference<=budget and closure<=budget and supported:
   ...
   return fine
  if depth>=max_depth:raise QuadratureBudgetError(f'Adaptive boundary depth budget exhausted at {lo:.17g}:{hi:.17g}: support={supported}, closure={closure}, difference={difference}',dict(begin=begin.tolist(),end=end.tolist(),alpha=float(alpha),beta=float(beta),lo=lo,hi=hi,depth=depth,budget_m2=budget,closure_m2=closure,difference_m2=difference,terms=terms))
  return merge(visit(lo,mid,left,depth+1),visit(mid,hi,right,depth+1))
 from .boundary_events import crossing_times
 cuts=crossing_times(begin,end,alpha,beta);stats['event_intervals']=len(cuts)-1
 result=None
 for lo,hi in zip(cuts,cuts[1:]):
  part=visit(lo,hi,sample(lo,hi),0)
