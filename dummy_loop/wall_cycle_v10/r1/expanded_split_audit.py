"""Expanded v4 material audit. No robot, transport pose or observation gate claim."""
...
def run_case(job):
    ...
    models=[SplitPressure(base.wall*base.wall_cell_area,base.blade*base.blade_cell_area,params=copy.deepcopy(base.p)) for _ in range(2)]
...
def main():
    ...
    (out/'manifest.json').write_text(json.dumps(dict(status='started',pid=os.getpid(),source_head=head,physics_version=SplitPressure.physics_version,singles=a.singles,sequences=a.sequences,steps=a.steps,spacings_m=[.00005,.000025],workers=a.workers,scope='material subsystem; no full environment/robot/transport claim',training_started=False),indent=2))
    monitor=subprocess.Popen(['C:/Users/28017/anaconda3/python.exe','-m','dummy_loop.wall_cycle_v09.telemetry','--out',str(out/'resources.jsonl'),'--pid',str(os.getpid()),'--interval','5'],creationflags=subprocess.CREATE_NO_WINDOW)
