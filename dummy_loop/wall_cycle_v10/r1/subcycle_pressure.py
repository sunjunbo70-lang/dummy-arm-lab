"""Diagnostic internal-pressure subcycling; geometry and laws unchanged.
Not frozen physics. Compare at original external sample spacings.
"""
import numpy as np
from .stationary_boundary_pressure import StationaryBoundaryPressure
class SubcyclePressure(StationaryBoundaryPressure):
    physics_version='v10r1.area_pressure_candidate.v8_6_pressure_subcycle4'
    def contact(self,center,angle,pitch,force):
        ...
        if travel>0:
            ...
            n=4*max(1,int(np.ceil(duration/.2)));h=duration/n
