"""Reproduce original case76 scene/controls with full candidate pressure and lift.
A diagnostic pair of resolutions, not the 100/50-case material gate.
"""
import argparse,copy,json,time,os
from pathlib import Path
import numpy as np
from .stationary_boundary_pressure import StationaryBoundaryPressure as CandidatePressure
...
def main():
 ...
 recipe=json.loads((Path(a.source)/'single_076_recipes.json').read_text())['recipes'][0];base,scene=make_scene(16)
 (out/'manifest.json').write_text(json.dumps(dict(pid=os.getpid(),source_head=head,physics_version=CandidatePressure.physics_version,steps=a.steps,recipe=recipe,scene=scene,training_started=False),indent=2))
 rows=[];previous=None
 for n in a.steps:
  s=CandidatePressure(base.wall*base.wall_cell_area,base.blade*base.blade_cell_area,params=copy.deepcopy(base.p));total=s.total();start=time.perf_counter();row=dict(steps=n);i=-1
  try:
