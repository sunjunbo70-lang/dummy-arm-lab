...
def case(task):
    ...
    d1=float(np.max(abs(arrays['after_401']-arrays['after_801']))*1000)
    d2=float(np.max(abs(arrays['after_801']-arrays['after_1601']))*1000)
    row=dict(seed=seed,levels=rows,max_diff_401_801_mm=d1,max_diff_801_1601_mm=d2,empirical_order=float(np.log2(d1/d2)) if d1>0 and d2>0 else None,local_single_thickness_tolerance_pass=d2<=.01)
...
def main():
    ...
    (out/'manifest.json').write_text(json.dumps(dict(physics_version=OrderedPressure.physics_version,workers=a.workers,samples=[401,801,1601],seeds=list(range(6)),status='started',scope='isolated six-case convergence; no G0 pass claim'),indent=2))
