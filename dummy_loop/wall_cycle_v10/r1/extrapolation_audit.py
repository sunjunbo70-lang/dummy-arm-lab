...
import numpy as np
from .extrapolated_inventory import ExtrapolatedPressure as OrderedPressure
...
def case(task):
    ...
    for n in (101,201,401):
        ...
        rows.append(dict(samples=n,seconds=time.perf_counter()-start,ledger_error_mL=abs(s.total()-total)*1e6,min_volume_m3=float(min(s.wall.min(),s.blade.min())),accepted=s.accepted_extrapolations,fallback=s.fallback_steps))
