...
def run(begin,end,alpha,beta,n):
 def area_at(x,a):
  raw=free_area(x,a)
 ...
 area=area_at(begin,alpha);wall=.002*area;blade=np.full(144,.001*.005**2);total=wall.sum()+blade.sum();lost=0.;correction=0.;maxiter=0;support_area=0.
 start=time.perf_counter()
 for i in range(n):
  u=i/n;v=(i+1)/n
  maps=integrate(begin+u*(end-begin),begin+v*(end-begin),alpha+u*(beta-alpha),alpha+v*(beta-alpha),intervals=2)
 ...
 return dict(status='completed',steps=n,seconds=time.perf_counter()-start,ledger_error_ml=float(abs(wall.sum()+blade.sum()+lost-total)*1e6),minimum_volume=float(min(wall.min(),blade.min(),lost)),max_iterations=maxiter,geometric_area_correction_m2=correction,endpoint_support_area_m2=support_area),wall,blade

def main():
 p=argparse.ArgumentParser();p.add_argument('--out',required=True);p.add_argument('--steps',nargs='+',type=int,default=[128,256]);a=p.parse_args();out=Path(a.out);out.mkdir(parents=True,exist_ok=False);head=snapshot(out);rows=[]
 for name,end,alpha,beta in [('axial',[.02,.25],0.,0.),('diagonal',[.02,.27],0.,0.),('rotation',[0.,.25],.31,.61)]:
  previous=None
  for n in a.steps:
   try:
    row,wall,blade=run(np.array([0.,.25]),np.array(end),alpha,beta,n)
 ...
 (out/'summary.json').write_text(json.dumps(dict(source_head=head,version=VERSION,rows=rows,training_started=False,full_material_gate=False,scope='finite blade boundary exchange, no pressure/bead/lift'),indent=2))
