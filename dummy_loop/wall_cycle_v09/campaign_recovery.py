...
def main():
 ...
 run_checked([PY,'-m','dummy_loop.wall_cycle_v09.dagger_straight','--checkpoint',str(previous/'training/straight_seed11_0100/checkpoints/bc.pt'),'--reach',str(REACH),'--out',str(dag),'--seed','60720','--episodes','40'],out/'logs/straight_dagger40.log')
 jobs=[dict(j,external=True) for j in prior if j['group']!='straight']
 recovery=job('straight',11,100,out)
 argv=recovery['argv'];argv[argv.index('--bc-iterations')+1]='6000';argv[argv.index('--dagger')+1]=str(dag)
 jobs.append(recovery)
 manifest.update(parent_campaign=str(previous),recovery_note='Original straight transfer BC gate failed at 72.3%. Straight-specific 40-episode DAgger plus 6000 BC updates; other completed jobs adopted without rerun. Adapted initialization is a confound, not a clean single-factor comparison.')
 atomic(out/'manifest.json',manifest)
