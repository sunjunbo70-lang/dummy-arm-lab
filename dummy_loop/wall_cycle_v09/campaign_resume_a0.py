...
def main():
 ...
 previous=BASE/'campaign_004_available_groups'
 prior=read(previous/'all_training_jobs.json')
 failed=[j for j in prior if j['status']!='complete']
 if len(failed)!=1 or failed[0]['id']!='RL0_11_400':raise RuntimeError('Unexpected failed groups')
 source=Path(failed[0]['out'])/'checkpoints/0328.pt'
 jobs=[dict(j,external=True) for j in prior if j['status']=='complete']
 hundred=[j for j in jobs if j['stage']==100]
 recovery=job('RL0',11,400,out,source)
 manifest.update(matrix_complete=False,missing_groups=['straight'],parent_campaign=str(previous),recovery_note='A0 terminal LOAD info self-reference fixed; exact resume from update328; failed partial rollout329 retained in original folder.',status='training')
 atomic(out/'manifest.json',manifest);atomic(out/'missing_groups.json',read(previous/'missing_groups.json'))
 wait_training([recovery],out)
 jobs.append(recovery);atomic(out/'all_training_jobs.json',jobs)
 if recovery['status']!='complete':manifest['status']='needs_attention';atomic(out/'manifest.json',manifest);return
 selected={}
 for j in jobs:
 ...
 manifest.update(status='completed_with_missing_ablation',completed_utc=time.time(),quality_success='See independent test per-episode metrics; completion does not imply quality thresholds passed.');atomic(out/'manifest.json',manifest)
