...
def main():
 ...
 previous=BASE/'campaign_003_straight_round2'
 prior=read(previous/'jobs.json')
 missing=[j for j in prior if j['status']!='complete']
 if len(missing)!=1 or missing[0]['group']!='straight':raise RuntimeError('Unexpected missing groups')
 gate=read(Path(missing[0]['out'])/'metrics/bc_gate.json')
 jobs=[dict(j,external=True) for j in prior if j['status']=='complete']
 atomic(out/'jobs.json',jobs)
 atomic(out/'missing_groups.json',{'straight':{'status':'failed_initialization_gate','final_gate':gate,'attempts':[str(BASE/n) for n in ['campaign_001','campaign_002_straight_recovery','campaign_003_straight_round2']],'PPO_updates':0,'additional_retries':False}})
 manifest.update(parent_campaign=str(previous),matrix_complete=False,missing_groups=['straight'],status='budget_decision')
 atomic(out/'manifest.json',manifest)
 hundred=list(jobs)
 ...
 print('Available groups delivered; straight ablation missing',out,flush=True)
