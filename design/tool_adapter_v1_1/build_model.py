...
for x in P['clamp_centres']:
    ...
    for y in (-P['clamp_bolt_y'],P['clamp_bolt_y']):
        ear_inner=15.0
        ear_outer=abs(y)+6.5
        ear_mid=math.copysign((ear_inner+ear_outer)/2,y)
        ear_span=ear_outer-ear_inner
        bottom=bottom.union(box(P['clamp_width'],ear_span,10,x,ear_mid,16))
        # Rib under extended lower ear: joins base edge to outer bolt seat.
        sign=1 if y>0 else -1
        rib=cq.Workplane('YZ',origin=(x-3,0,0)).polyline([(sign*18,7),(sign*ear_outer,16),(sign*18,16)]).close().extrude(6)
        bottom=bottom.union(rib)
        cap=cap.union(box(P['clamp_width'],ear_span,5,x,ear_mid,P['handle_axis_z']+P['split_gap']))
        # Thickened inner cap ear; leave flat screw-head seat outside |Y|=26.
        cap=cap.union(box(P['clamp_width'],11,2,x,math.copysign(20.5,y),P['handle_axis_z']+P['split_gap']+5))
...
# Tool insertion path: conservative straight 10 mm shaft from screw head
# seat to Z=120, then 30 mm handle to Z=200. No assumption about hand reach.
access=[]
for x in P['clamp_centres']:
    for y in (-P['clamp_bolt_y'],P['clamp_bolt_y']):
        shaft=bore(10,120-31.81,x,y,31.81)
        grip=bore(30,80,x,y,120)
        sweep=shaft.union(grip)
        volumes={n:sweep.val().intersect(o.val()).Volume() for n,o in [('main',base),('tail_cap',caps[0]),('tip_cap',caps[1]),('handle',handle),('blade',blade),('neck',neck),('camera',camera)]}
        assert max(volumes.values())<1e-5,volumes
        access.append({'screw_xy_mm':[x,y],'shaft_diameter_mm':10,'shaft_z_mm':[31.81,120],'handle_diameter_mm':30,'handle_z_mm':[120,200],'intersection_mm3':volumes})
audit['screwdriver_access']=access
...
def setup(ax,title,lim=(-55,80,-45,92,0,80)):
...
setup(ax,'V1.1 adapter: assembled print parts')
...
setup(ax,'Exploded: 2 separate caps, 4 clamping screws',(-40,40,-45,85,0,85))
...
for o,col in [(base,'#368bad'),(caps[0],'#66c4d7'),(caps[1],'#66c4d7'),(blade,'#bfc4c8'),(handle,'#c59553'),(camera,'#303942')]:draw(ax,o,col)
for x in P['clamp_centres']:
 for y in (-32,32):draw(ax,bore(10,88.19,x,y,31.81),'#f5a623',.3)
setup(ax,'V1.1: four clear screwdriver paths (dia10 mm)',(-55,80,-45,92,0,125))
fig.tight_layout();fig.savefig(ROOT/'screwdriver_access.png',dpi=160);plt.close(fig)
