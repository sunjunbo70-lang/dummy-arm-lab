...
class USBBulkTransport(fibre.protocol.PacketSource, fibre.protocol.PacketSink):
  ...
  def init(self):
    ...
    # This diagnostic client may claim the native interface only, never COM6.
    all_compatible_interfaces = custom_interfaces
  ...
  def process_packet(self, usbBuffer):
    try:
      # Zero means infinite in libusb: a stalled OUT transfer must not freeze
      # the commissioning worker and prevent its serial STOP cleanup.
      ret = self.epw.write(usbBuffer, 200)
  ...
  def get_packet(self, deadline):
    try:
      ...
      if remaining <= 0:
        raise TimeoutError()
      timeout = max(int(remaining * 1000), 1)
    ...
    for usb_device in devices:
      try:
        ...
        if ex.errno == 13:
          ...
          raise RuntimeError("Native USB interface busy; diagnostic will not reset the robot") from ex
        else:
