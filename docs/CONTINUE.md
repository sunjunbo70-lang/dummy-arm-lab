...
最新一轮是「整片连续强化学习 v0.3」：`experiments/v0.3/r0/records/2026-09-22_wall_cycle_v03/`。
它用物理化的砂浆模型（不写死任何倾斜角规则，让模型自己学）+ 真实 Dummy V2 机械臂在 MuJoCo 里联合仿真。
完整的改动原因、和上一版（v0.2，`experiments/v0.2/r0/records/2026-09-22_wall_cycle_rl/`）逐项对比，在
`experiments/v0.3/r0/design/2026-09-22_wall_cycle_v0.3.md`——**这份文件专门是为了让接手的人知道"为什么这么改、后续迭代时哪些不能改回去"**，改代码前务必先看。

当前最好的模型：`experiments/v0.3/r0/records/2026-09-22_wall_cycle_v03/raw/technique_teacher/policy.npz`（立起手法预热那组，测试集回报 −35.7，优于平刀预热那组）。

双击仓库根目录的 `整片连续强化学习可视化.cmd`，会在 MuJoCo 窗口里播放这个模型在联合仿真下的一局（真实机械臂 + 墙面模型，不是折线示意图）。

## 3. 还没解决的问题（按 `experiments/v0.3/r0/records/2026-09-22_wall_cycle_v03/README.md`「局限」排序）
...
```bash
python tools/simulation/v2_trowel_reach.py --pitches 0 --distances 0.28 0.30 0.33 0.35 0.38 --out experiments/v0.1/r0/runs/reach
...
python -m dummy_loop.wall_cycle.train --out experiments/v0.3/r0/runs/v03_flat --teacher-style flat
python -m dummy_loop.wall_cycle.train --out experiments/v0.3/r0/runs/v03_technique --teacher-style technique
python tools/simulation/wall_cycle_report.py --flat experiments/v0.3/r0/runs/v03_flat --technique experiments/v0.3/r0/runs/v03_technique --out experiments/v0.3/r0/runs/v03_report
```
